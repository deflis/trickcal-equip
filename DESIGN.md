...
## Typography
- **Display Font**: Zen Kaku Gothic New — Google Fonts よりロード
- **Body Font**: Noto Sans JP — Google Fonts よりロード
- **Code Font**: JetBrains Mono — Google Fonts よりロード

ディスプレイおよび見出しのテキストには、太字の Zen Kaku Gothic New を使用し、文字間隔をわずかに詰め（-0.01em 〜 -0.02em）、力強くモダンな印象を与えます。本文および UI テキストには Noto Sans JP の Regular および Medium ウェイトを使用し、高い視認性を確保します。幾何学的な見出しフォントと、親しみやすく読みやすい本文フォントのコントラストが、洗練されたエディトリアルな雰囲気を作り出します。コードブロック、API キー、CLI コマンドには JetBrains Mono を使用します。
...
- Do use Zen Kaku Gothic New for headings and Noto Sans JP for body — never swap them
